...
class Fa extends REST_Controller {
    ...
    public function __construct() {
     ...
     function accept_post() {
        ...
        if ($result_set->num_rows() > 0) {
            $row = $result_set->row();
            if($row->fa_status==0 || $row->fa_status==2){
               if($status==1)
                     $status_msg = "Approved";
               else  if($status==2)
                     $status_msg = "Rejected";
               ...
               $action_status =$this->model_all->update(array("fa_status" => $status,"fa_status_by"=>$user), array("id" => $order), 'seller_orders');
               if($action_status){
                   $result["status"] = 1;
                   if($status==1)
                     $status_msg = "Approved";
                 ...
                 //    $this->model_all->track_parent_order($order,$status_msg);
        ...
        if($notify_data["dealer"]["fcm_key"]!=""){
            ...
            $body = "Your order ".$order_string." has ".$status_msg." by Depot Manager"; /// Message goes here;
            $payload['body'] =  $body;
        ...
        if($notify_data["se"]["fcm_key"]!=""){
            ...
            $data = array();
            $payload['title'] = "Welcome to Nova";
            $body =  "The order ".$order_string." has ".$status_msg." by Depot Manager which was placed by ".$notify_data["dealer"]["company_name"]."(".$notify_data["dealer"]["dealer_code"].")"; /// Message goes here
            $payload['body'] = $body;
            $payload['icon'] = "";  // Name of the icon in the play store
            $payload['click_action'] = "mainactivity";  // For android click activity
            $data['id'] = $order_id;  // For custom value if any
        ...
        if($notify_data["admin"]["fcm_key"]!=""){
            ...
            $payload['click_action'] = "mainactivity";  // For android click activity
            $data['id'] = $order_id;  // For custom value if any
            $payload['to'] = $notify_data["admin"]["fcm_key"];   // Receiver FCM id
            $data['role'] = "ADMIN";   // For custom value if any
            $this->model_all->save(array("notification"=>$body,"notify_type"=>"approved","user_role"=>"ADMIN","user_id"=>$notify_data["admin"]["id"],"branch"=>$row->branch_id,"is_seen"=>"N","notifiy_on"=>date("Y-m-d H:i:s"),"related_id"=>$order_id),"notifications");
            $this->fcm->send($payload['to'], $payload, $data);
    ...
    function orders_get() {
        ...
        if ($status == "Cancelled") {
            ...
            $condition .= " and o.status='Ordered' and o.fa_status='0' and o.is_transfered!='2'";
        } else if ($status == "Approved") {
            $condition .= " and ((o.status='Ordered' and o.fa_status='1' and o.fa_status_by='$user') or (o.status='Accepted' and o.admin_status='1') or (o.status NOT IN ('Cancelled','Rejected') and (o.fa_status='1' or o.admin_status='1')) )";  // and delivery_recieved='1'
            ...
            $condition .= " and ((o.fa_status='2' and o.fa_status_by='$user') or (o.fa_status='1' and o.fa_status_by='$user' and o.admin_status='2') or (o.fa_status='2' and o.fa_status_by='$user' and o.admin_status='2'))";
        ...
        if ($result_set->num_rows() > 0) {
            ...
            foreach ($result_set->result_array() as $row) {
                ...
                if($row["orderedby"]!=$row["created_by"]){
                     ...
                     $emp_qry = $this->model_all->getTableDataFromQuery("select e.first_name as emp_name,e.mobile,b.name as branch_name from employees e,branches b where find_in_set(b.id,e.branch) and e.id='$row[created_by]'");
                     if($emp_qry->num_rows()>0){
                          $emp_rs=$emp_qry->row();
                          $row["takenby_name"] = $emp_rs->emp_name.$parent_string;
                ...
                if($row["dealer_contact"]==0 && $row["dealer_contact2"]!=""){
                   $row["dealer_contact"] = $row["dealer_contact2"];
