...
//echo $query;
    ...
    function dealers_get() {
        ...
        if ($branch!= "") {
         ...
         $query = "select s.id,s.first_name,s.last_name,s.mobile,s.contact1,s.dealer_code,s.branch,s.status,s.email,s.wallet,s.company_name,a.address as seller_adress $branch_fields from sellers s,addresses a $branch_table where   s.status='1' and  a.user_id=s.id and a.user_role='DEALER' and s.company_name!='' $condition ";

        echo $query." order by s.company_name ";
        ...
        if ($result_set->num_rows() == 0) {
            ...
            if ($employee > 0) {
                ...
                if ($employee > 0) {
                    ...
                    $this->model_all->save(array("dp" => 'noimage.png', "username" => $uniq_code, "password" => md5($this->model_all->randomPassword()), "role" => $role_nm, "pkid" => $employee, "status" => '1', "createdon" => $dt, "modifiedon" => $dt, "createdby" => $user, "modifiedby" => $user), "app_users");
                }
            }
