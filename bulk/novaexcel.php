...
function getClassDetails($column, $id) {
    return mysql_data("select " . $column . " from  classes where class_id='$id'", "$column");
}
function getColumnDetails($table, $column, $wcol, $id) {
    return mysql_data("select " . $column . " from  ".$table." where ".$wcol."='$id'", "$column");
}
...
function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$aca_year = $_SESSION['acyear'];

		
$prefix = mysql_data("select school_code from about_school", "school_code");
		for($row=2;$row<=$this->rowcount($sheet);$row++) {
			$pswd = rand(11111, 99999);
$pswd_p = rand(22222, 99999);
				$stdid = $this->val($row,2,$sheet);
				if($stdid!=""){
				$class = $this->getColumnDetails('classes','class_id','class',$this->val($row,3,$sheet));
				$sec = $this->getColumnDetails('sections', 'sec_id', 'section', $this->val($row,4,$sheet));
				$fname = $this->val($row,5,$sheet);
				$lname = $this->val($row,6,$sheet);
				$smobile = $this->val($row,7,$sheet);
				$semail = $this->val($row,8,$sheet);
				$gender = $this->val($row,9,$sheet);
				$address = $this->val($row,10,$sheet);
				$adno = $this->val($row,11,$sheet);
				$dob = date('Y-m-d',strtotime($this->val($row,12,$sheet)));
				if($this->val($row,13,$sheet)==''){
				    $dojj = "";
				}else{
				$dojj = date('Y-m-d',strtotime($this->val($row,13,$sheet)));
				}
				$bgroup = $this->getColumnDetails('blood_groups','blood_group_id','blood_group',$this->val($row,14,$sheet));
				$caste = $this->getColumnDetails('cast','cast_id','cast',$this->val($row,15,$sheet));
				$academic = $this->getColumnDetails('academics','ac_id','ac_year',$this->val($row,16,$sheet));
				$aadhar = $this->val($row,17,$sheet);
				$pname = $this->val($row,18,$sheet);
				$pmobile = $this->val($row,19,$sheet);
				$altmob = $this->val($row,20,$sheet);
				$mname = $this->val($row,21,$sheet);
				$gname = $this->val($row,22,$sheet);
				$pemail = $this->val($row,23,$sheet);
				$hostel = $this->val($row,24,$sheet);
				$transport = $this->val($row,25,$sheet);
				$reimbersement = $this->val($row,26,$sheet);
				$management = $this->val($row,27,$sheet);
				$rnum = $this->val($row,28,$sheet);
				$medium = $this->getColumnDetails('mediums','medium_id','medium',$this->val($row,29,$sheet));
				$status = $this->val($row,30,$sheet);
				
				
				//echo "select prt_id from parent_reg where prt_mb='$pmobile'";
    $mobile_count = mysql_data("select prt_id from parent_reg where prt_mb='$pmobile'", "prt_id");
	//echo $mobile_count."fsgrd";
    if (empty($mobile_count)) {
       //echo "INSERT INTO parent_reg (prt_id, prt_name, lname,mother_name,guardian_name, prt_mb, alt_mb, prt_mail, prt_psw) VALUES ('NULL' , '$pname' , '$lname' ,'$mname' , '$gname' , '$pmobile', '$altmob' , '$pemail' , '$pswd_p')";
		$preg = mysql_query("INSERT INTO parent_reg (prt_id, prt_name, lname,mother_name,guardian_name, prt_mb, alt_mb, prt_mail, prt_psw) VALUES ('NULL' , '$pname' , '$lname' ,'$mname' , '$gname' , '$pmobile', '$altmob' , '$pemail' , '$pswd_p')");
            $pid = mysql_insert_id(); //parent_id
			 $pgenerate = $prefix . "P" . $aca_year . "" . $pid;     //$pname.$lname.class_name($class).sec_name($sec).$rnumb;
        mysql_query("update parent_reg set prt_uid='$pgenerate' where prt_id='$pid'");
    }//if($mobile_count>0 && $existParent=="Y")
    else{
	$pid=$mobile_count;	
		}
	//echo $pid."sfrsgrst";
    //$adno = mysql_data("select MAX(adno) as adno from student", "adno");
   // $adno = $adno + 1;
   $stu_count = mysql_data("select count(*) from student where std_id='$stdid'", "count(*)");
       if ($pid!= 0 && $stu_count==0) {
        $insertQry = mysql_query("INSERT INTO student(adno, name, lname, dob, gender,aadhar, caste, academic, hostel, transport, reimbursement,management, class, section, Roll_num, doj, fatherName, motherName, guardianName ,presentAddress, permanentAddress, contactNumber, emailId, pswd, image,`std_id`,`f_id`,`parent_id`, `blood_group`, `medium`,`status`)VALUES('$adno' , '$fname' , '$lname' , '$dob' ,'$gender' ,'$aadhar' ,'$caste' ,'$academic' ,'$hostel' ,'$transport' ,'$reimbersement','$management', '$class' , '$sec' , '$rnum' , '$dojj' ,'$pname' , '$mname' , '$gname' , '$address' , '$address' , '$smobile' , '$semail' , '$pswd', '','$stdid','$_SESSION[luser_id]','$pid', '$bgroup', '$medium','$status')");
    }else{
    	echo $stdid;
    }
  

    if (($sid = mysql_insert_id()) > 0) {
      if($status=="Y"){
        $msg = 'Student Successfully registered with admission number '.$adno;
        $year_id = $this->getClassDetails('year_id', $class);
        $class_qry = mysql_query("select * from  classes where class_id='$class'");
        $class_rs = mysql_fetch_object($class_qry);
        $feeTermQry = mysql_query("SELECT f.*,ft.optional,ft.fee_type,ft.is_management FROM `fee_master` f,fee_types ft where f.ac_id='$academic' and f.course_id='$class_rs->course_id' and f.branch_id='$class_rs->group_id' and ft.fee_type_id=f.fee_type_id");
        if (mysql_num_rows($feeTermQry) > 0) {
            while ($feeTermRs = mysql_fetch_object($feeTermQry)) {
                if ($feeTermRs->year_id == 0 || ($feeTermRs->year_id != 0 && $feeTermRs->year_id == $class_rs->year_id)) {
                    if ($feeTermRs->semester_id == 0 || ($feeTermRs->semester_id != 0 && $feeTermRs->semester_id == $class_rs->semister_id)) {
                       if($feeTermRs->optional=='N' || ($feeTermRs->is_management=='Y' && $management=='Y'))
                        mysql_query("INSERT INTO `student_fee_master` (`student_master_id`, `student_id`, `fee_master_id`, `discount`) VALUES (NULL, '$sid', '$feeTermRs->fee_master_id', '0.00');");
                    }
                }
            }
        }
